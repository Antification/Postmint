...
```javascript
// build-mint.js — Postmint v6 (shared contract; on-chain SVG card; _mini avatar keeps calldata submit-safe)
...
// IMPORTANT: the avatar deliberately uses Twitter's `_mini` (48px) variant with a HARD 8KB cap. The card
// draws the avatar at 56px so a 48px source is native resolution — no visible loss. Large avatars (the
// old 400KB cap) ballooned calldata past 40K hex chars, which gets truncated in transit to the signer
// and reverts on simulation. Do NOT raise the cap.
const xmlEsc = (s) => String(s || '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
async function avatarDataUri(url) {
  const candidates = [];
  if (url) {
    const mini = url.replace(/_(?:bigger|normal|\d+x\d+)\.(jpg|jpeg|png|webp)/i, '_mini.$1');
    if (mini !== url) candidates.push(mini);
    candidates.push(url);
  }
  for (const u of candidates) {
    try {
      ...
      if (b.length > 8000) continue; // hard cap: keeps total calldata ~10K hex chars (submit-safe)
...
function buildCardSvg(t, avatarUri) {
  ...
  const maxChars = Math.floor((W - PAD * 2) / (fontSize * 0.52));
  const lines = wrapText(t.text || '', maxChars);
  const headerH = PAD + AV + 20;
  const H = headerH + lines.length * lineH + PAD + 28;
  const cx = PAD + AV / 2;
...
if (existing && existing > 0n) {
  ...
  console.log(JSON.stringify({
    alreadyMinted: true, statusId, tokenId: existing.toString(), minter,
    openseaToken: `https://opensea.io/assets/base/${CONTRACT}/${existing}`,
    openseaCollection: `https://opensea.io/assets/base/${CONTRACT}`, resolvedFrom,
  }, null, 2));
  process.exit(0);
}
...
if (animationUrl) tokenMeta.animation_url = animationUrl;
const tokenUri = dataUri(tokenMeta);

// --- RE-CHECK right before building the tx ---
...
if (reCheck && reCheck > 0n) {
  ...
  console.log(JSON.stringify({
    ...
    openseaCollection: `https://opensea.io/assets/base/${CONTRACT}`, resolvedFrom, raced: true,
  }, null, 2));
  process.exit(0);
}
...
console.log(JSON.stringify({
  ...
  dataLen: data.length,
  expectedTokenId: nextId.toString(), statusId, resolvedFrom,
...
**CALLDATA INTEGRITY CHECK (mandatory):** the script output includes `dataLen` — the exact character length of `tx.data`. The `data` string you pass to `submit_raw_transaction` must be COMPLETE and byte-exact: same length, same content, no truncation, no re-typing. A truncated copy encodes garbage arguments and fails simulation (`simulation_reverted`), typically showing a suspiciously small `dataBytes` in the error (e.g. 512 when `dataLen` was 10,000+). If a submit fails this way, do NOT retry blindly — re-run the build script and re-copy the full `data` string exactly as emitted.
...
If `resolved: true`, reply with path C using `minter` + `openseaToken`. If `resolved: false`, the revert was NOT a duplicate (e.g. truncated calldata / gas / RPC) — check the CALLDATA INTEGRITY CHECK in Step 2 first, then report the real error and offer to retry.
...
- **`simulation_reverted` with a SMALL `dataBytes` in the error (e.g. 512) on a text-only mint:** the calldata was TRUNCATED between the build script and `submit_raw_transaction` — NOT a contract bug and NOT a duplicate post. The submitted `data` must match the script's emitted `dataLen` exactly (see Step 2's CALLDATA INTEGRITY CHECK). Older skill versions embedded avatars up to 400KB, producing 40K+ hex-char calldata that routinely broke in transit; the current script caps the avatar at the `_mini` variant / 8KB, keeping calldata ~10K hex chars. Fix: make sure you're running the CURRENT build-mint.js from this SKILL.md, re-run it, and submit the full `data` string unmodified. Run `resolve-winner.js` first if you're unsure whether the revert was a duplicate — `resolved:false` means it was NOT a duplicate (likely truncation).
- **Tx reverts with "post already minted":** you lost a first-come-first-served race. This is EXPECTED. Do NOT show the raw revert. Run Step 2b (`resolve-winner.js`) and reply with OUTPUT CONTRACT **path C** — the winning minter + OpenSea token link. Only if `resolve-winner.js` returns `resolved:false` is the revert a genuine (non-duplicate) failure worth surfacing (check for truncated calldata first).
...
- **Text-only SVG card is missing the avatar (shows a plain circle):** no avatar variant fit the 8KB embed cap or the fetch failed, so the card drew a placeholder circle. The text, name, handle, and date still render. Re-run to retry the avatar fetch if a transient failure is suspected. Do NOT work around this by raising the cap — that reintroduces the truncated-calldata revert.
- **Text-only SVG card text looks cramped or overflows the card width:** the greedy word-wrap uses an empirical avg glyph width (`fontSize * 0.52`) to pick chars-per-line. It does not chop text (height grows to fit). To widen the margin, lower the `0.52` factor in `wrapText`'s `maxChars` calc.
